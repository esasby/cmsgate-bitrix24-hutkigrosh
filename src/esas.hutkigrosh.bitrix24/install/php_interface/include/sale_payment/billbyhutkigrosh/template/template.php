...
<?
/**
 * Дополнительная секция для ХГ
 * @var \esas\cmsgate\hutkigrosh\view\client\CompletionPanelHutkigroshBitrix24 $completionPanel
 */
$completionPanel = $params['completionPanel'];
if ($completionPanel) {
    if (array_key_exists(\esas\cmsgate\hutkigrosh\ConfigFieldsHutkigrosh::instructionsSection(), $params))
        $completionPanel->setInstructionsSectionEnabled($params[\esas\cmsgate\hutkigrosh\ConfigFieldsHutkigrosh::instructionsSection()]  == "Y");
    if (array_key_exists(\esas\cmsgate\hutkigrosh\ConfigFieldsHutkigrosh::qrcodeSection(), $params))
        $completionPanel->setQRCodeSectionEnabled($params[\esas\cmsgate\hutkigrosh\ConfigFieldsHutkigrosh::qrcodeSection()]  == "Y");
